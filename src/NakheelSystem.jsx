...
const productBarcodes = (p) => [p.bc, ...((p.barcodes) || [])].filter(Boolean);
const matchesBarcode = (p, code) => productBarcodes(p).some(b => b === code);
const matchesBarcodePartial = (p, q) => productBarcodes(p).some(b => b.includes(q));
...
function CommandPalette({ open, onClose, products, customers, invoices, hubs, goPage, setSearchIntent, currency }) {
