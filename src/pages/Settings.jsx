    ...
    <>
      ...
      {tab === "printers" && (
        <Card className="nk-card-hover">
          ...
          {/* عدد نسخ الطباعة — أجهزة الكاشير غالباً تعمل بوضع "طباعة صامتة" لا
              تُظهر نافذة طباعة المتصفح إطلاقاً (فلا مكان لتعديل عدد النسخ من
              هناك)، وتعتمد بصمت على آخر عدد نسخ محفوظ لهذه الطابعة في النظام
              — قد يكون 2 دون علم المستخدم. هذا الإعداد يتحكم بالعدد من داخل
              النظام نفسه بغض النظر عن أي إعداد خارجي مخفي */}
          <div style={{ marginTop: 12, borderTop: `0.5px solid ${C.bc}`, paddingTop: 12 }}>
            <Field label="عدد نسخ الطباعة الافتراضي لكل فاتورة/وصل">
              <Inp type="number" min="1" max="5" value={settings.printCopies || 1} onChange={e => set("printCopies", Math.max(1, Math.min(5, parseInt(e.target.value) || 1)))} style={{ maxWidth: 100 }} />
            </Field>
            <div style={{ fontSize: 10.5, color: C.mt, marginTop: -6 }}>يمكن تغيير العدد لطباعة واحدة بعينها من نافذة المعاينة نفسها عند الطباعة.</div>
          </div>
