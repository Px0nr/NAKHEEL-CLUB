...
// تعديل زبون موجود: مرّر customer فتتحوّل النافذة تلقائياً لوضع التعديل (اسم/هاتف
// فقط، بلا حقول الدين الافتتاحي التي تخص الإنشاء لمرة واحدة) — كان لا توجد أي
// وسيلة لتصحيح اسم أو رقم واتساب بعد تسجيل الزبون سوى حذفه وإعادة إدخاله
export default function NewCustomerModal({ ctx, customer, onClose, onCreated }) {
  const { customers, setCustomers, setInvoices, user, showToast } = ctx;
  const cur = ctx.settings?.currency || "د.ل";
  const isEdit = !!customer;
  const [f, setF] = useState({ name: customer?.name || "", phone: customer?.phone || "", openingDebt: "", debtDate: todayISO() });
  const save = () => {
    ...
    if (isEdit) {
      setCustomers(cs => cs.map(x => x.id === customer.id ? { ...x, name: f.name.trim(), phone: f.phone, wa } : x));
      showToast("تم تحديث بيانات الزبون");
      onCreated ? onCreated({ ...customer, name: f.name.trim(), phone: f.phone, wa }) : onClose();
      return;
    }
  ...
  return (
    <Modal title={isEdit ? `تعديل بيانات — ${customer.name}` : "تسجيل زبون جديد"} onClose={onClose} width={440}>
      ...
      <div style={{ fontSize: 11, color: C.gdd, background: C.gold + "12", borderRadius: 8, padding: ".55rem .75rem", marginBottom: 12 }}>{isEdit ? "تغيير رقم الواتساب يُستخدم فوراً في كل التقارير والفواتير المستقبلية لهذا الزبون." : "لا يمكن إضافة زبون بدون رقم واتساب — يُستخدم لإرسال الفواتير والتقارير."}</div>
      <div style={{ display: "flex", gap: 8 }}><Btn gold onClick={save} style={{ flex: 1, justifyContent: "center" }}>✓ {isEdit ? "حفظ التعديلات" : "حفظ الزبون"}</Btn><Btn onClick={onClose}>إلغاء</Btn></div>
    </Modal>
  );
}
